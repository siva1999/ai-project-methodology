...
def model(X_train,categorical_columns,continuous_columns,onehot_encoder,y_train,scaler,encoder):
    ...
    X_train_processed = pd.concat([pd.DataFrame(X_train_scaled, columns=continuous_columns),
...
def build_model(data: pd.DataFrame) -> dict:
    ...
    X_train_scaled = model(X_train,categorical_columns,continuous_columns,encoder,y_train,scaler,encoder)
